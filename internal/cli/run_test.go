...
type providedPlanCredentials struct {
	payload map[string]string
}

func (p *providedPlanCredentials) Resolve(context.Context, domain.DesiredState, bool) (map[string]string, error) {
	return nil, errors.New("unexpected fallback credential resolution")
}

func (p *providedPlanCredentials) ResolveProvidedForPlan(_ context.Context, _ domain.DesiredState, _ []reconcile.Action, payload map[string]string) (map[string]string, error) {
	p.payload = payload
	return payload, nil
}
...
func TestRunJSONEvents_EmitsRedactedProgressAndTerminalResult(t *testing.T) {
	canary := "wizard-event-secret-canary"
	service := &fakeService{hasPlan: true, plan: oneActionPlan(), progress: []reconcile.ProgressEvent{
		{Phase: reconcile.ProgressStarted, Action: reconcile.ActionSyncContent},
		{Phase: reconcile.ProgressCompleted, Action: reconcile.ActionSyncContent},
	}}
	...
	runner := Runner{
		Service:     service,
		Credentials: fixedCredentials{"TEAMKIT_SOURCE_TOKEN": canary},
		In:          strings.NewReader(""),
		Out:         &stdout,
		Err:         &stderr,
	}

	if code := runner.Run(context.Background(), append(linuxArgs("apply"), "--json-events")); code != ExitOK {
		t.Fatalf("exit=%d stderr=%q", code, stderr.String())
	}
	if strings.Contains(stdout.String()+stderr.String(), canary) {
		t.Fatalf("wizard event leaked secret: stdout=%q stderr=%q", stdout.String(), stderr.String())
	}
	...
	if len(events) != 3 {
		t.Fatalf("events=%#v want two progress events and one result", events)
	}
	if events[0].SchemaVersion != 1 || events[0].Event != "progress" || events[0].Phase != string(reconcile.ActionSyncContent)+":started" {
		t.Fatalf("first event=%#v", events[0])
	}
	if events[1].SchemaVersion != 1 || events[1].Event != "progress" || events[1].Phase != string(reconcile.ActionSyncContent)+":completed" {
		t.Fatalf("second event=%#v", events[1])
	}
	if events[2].SchemaVersion != 1 || events[2].Event != "result" || events[2].Command != "apply" || events[2].Status != string(reconcile.StatusReady) {
		t.Fatalf("terminal event=%#v", events[2])
	}
	if !reflect.DeepEqual(events[2].Credentials, []string{credentials.GitLabUsername, credentials.GitLabToken}) {
		t.Fatalf("terminal credential names=%v", events[2].Credentials)
	}
}

func TestRunJSONEvents_EmitsRedactedFailure(t *testing.T) {
	canary := "wizard-event-error-canary"
	service := &fakeService{hasPlan: true, plan: oneActionPlan(), applyErr: errors.New("provider rejected " + canary)}
	var stdout, stderr bytes.Buffer
	runner := Runner{
		Service:     service,
		Credentials: fixedCredentials{"TEAMKIT_SOURCE_TOKEN": canary},
	...
	if strings.Contains(stdout.String()+stderr.String(), canary) {
		t.Fatalf("wizard failure leaked secret: stdout=%q stderr=%q", stdout.String(), stderr.String())
	}
	var event struct {
		SchemaVersion int    `json:"schema_version"`
		Event         string `json:"event"`
	...
	if err := json.Unmarshal(stdout.Bytes(), &event); err != nil {
		t.Fatalf("decode failure: %v stdout=%q", err, stdout.String())
	}
	if event.SchemaVersion != 1 || event.Event != "failure" || event.Code == "" || strings.Contains(event.Message, canary) {
...
func TestRunApplyCredentialsStdin_UsesBoundedPayloadOutsideArguments(t *testing.T) {
	service := &fakeService{}
	credentialSource := &providedPlanCredentials{}
	canary := "stdin-secret-canary"
	var stdout, stderr bytes.Buffer
	runner := Runner{Service: service, Credentials: credentialSource, In: strings.NewReader(`{"schema_version":1,"credentials":{"TEAMKIT_SOURCE_TOKEN":"` + canary + `"}}`), Out: &stdout, Err: &stderr}
	args := []string{
		"apply", "--non-interactive", "--credentials-stdin", "--os", "linux", "--app", "codex", "--app-installed=true",
		"--kit-home", "/kit", "--project", "wms", "--role", "developer", "--toolchain", "cc_1c_skills", "--update", "none",
	}
	if code := runner.Run(context.Background(), args); code != ExitOK {
		t.Fatalf("exit=%d stdout=%q stderr=%q", code, stdout.String(), stderr.String())
	}
	if credentialSource.payload["TEAMKIT_SOURCE_TOKEN"] != canary || service.secrets["TEAMKIT_SOURCE_TOKEN"] != canary {
		t.Fatalf("payload=%#v secrets=%#v", credentialSource.payload, service.secrets)
	}
	if strings.Contains(stdout.String()+stderr.String(), canary) || strings.Contains(strings.Join(args, "\x00"), canary) {
		t.Fatalf("credential leaked to observable output or arguments")
	}
}

func TestRunUpdateCredentialsStdin_UsesVerifiedEnvironmentAndBoundedPayload(t *testing.T) {
	home := filepath.Join(testutil.TempDir(t), "kit")
	service := &fakeService{}
	credentialSource := &providedPlanCredentials{}
	inspector := &fakeInspector{byHome: map[string]inspectResult{
		home: {verified: verifiedEnvironment(t, home, "wms"), state: environment.Ready},
	}}
	canary := "update-stdin-secret-canary"
	var stdout, stderr bytes.Buffer
	runner := Runner{Service: service, Credentials: credentialSource, Environments: inspector, In: strings.NewReader(`{"schema_version":1,"credentials":{"TEAMKIT_SOURCE_TOKEN":"` + canary + `"}}`), Out: &stdout, Err: &stderr}
	args := []string{"update", "--non-interactive", "--credentials-stdin", "--kit-home", home, "--target", "content"}
	if code := runner.Run(context.Background(), args); code != ExitOK {
		t.Fatalf("exit=%d stdout=%q stderr=%q", code, stdout.String(), stderr.String())
	}
	if inspector.inspectCalls != 1 || service.plans != 1 || service.verifiedUpdates != 1 || service.updates != 1 {
		t.Fatalf("inspect=%d service=%#v", inspector.inspectCalls, service)
	}
	if credentialSource.payload["TEAMKIT_SOURCE_TOKEN"] != canary {
		t.Fatalf("payload=%#v", credentialSource.payload)
	}
	if strings.Contains(stdout.String()+stderr.String(), canary) || strings.Contains(strings.Join(args, "\x00"), canary) {
		t.Fatalf("credential leaked to observable output or arguments")
	}
...
func TestRunWizardState_AddWritesOnlyPublicCheckpoint(t *testing.T) {
	root := filepath.Join(testutil.TempDir(t), "workspace")
	var stdout, stderr bytes.Buffer
	runner := Runner{Out: &stdout, Err: &stderr, GOOS: "windows"}
	args := []string{
		"wizard-state", "--json", "--non-interactive", "--mode", "add", "--os", "windows", "--app", "codex", "--app-installed=true",
		"--kit-home", root, "--project", "asku", "--role", "architect", "--toolchain", "cc_1c_skills", "--update", "none",
	}
	if code := runner.Run(context.Background(), args); code != ExitOK {
		t.Fatalf("exit=%d stderr=%q", code, stderr.String())
	}
	...
	if result.Command != "wizard-state" || result.Mode != "add" || result.CheckpointPath != workspace.WizardEnvPath(root) {
	...
	for _, forbidden := range []string{"TOKEN", "TEAMKIT_SOURCE_TOKEN_VALUE", "TEAMKIT_PUBLIC_PROVIDER_API_KEY_VALUE"} {
		if strings.Contains(string(contents), forbidden) || strings.Contains(stdout.String(), forbidden) || strings.Contains(stderr.String(), forbidden) {
			t.Fatalf("public checkpoint leaked %q", forbidden)
		}
	}
}

func TestRunWizardState_AddRejectsForeignWorkspace(t *testing.T) {
	root := filepath.Join(testutil.TempDir(t), "workspace")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, "keep.txt"), []byte("unrelated"), 0o600); err != nil {
		t.Fatal(err)
	}
	var stdout, stderr bytes.Buffer
	runner := Runner{Out: &stdout, Err: &stderr, GOOS: "windows"}
	...
	if code := runner.Run(context.Background(), args); code == ExitOK {
		t.Fatalf("exit=%d stdout=%q stderr=%q; want failure", code, stdout.String(), stderr.String())
	}
	if _, err := os.Lstat(workspace.WizardEnvPath(root)); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("foreign workspace received checkpoint: %v", err)
	}
}
...
func TestRunBareHelpExitsSuccessfully(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "teamkit plan|apply|status|retry|update|wizard-state|user-check|version") {
		t.Fatalf("stdout=%q", stdout.String())
	}
}
