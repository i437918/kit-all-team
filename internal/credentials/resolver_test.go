...
func TestResolverResolveProvidedForPlan_SavesOnlyRequiredPayload(t *testing.T) {
	desired := nonHermesDesired(t)
	store := &fakeStore{values: map[string]string{GitCAFile: "C:/private/ca.pem"}}
	resolver := Resolver{
		Home:  func(domain.DesiredState) (string, error) { return testutil.TempDir(t), nil },
		Store: func(string) (SecretStore, error) { return store, nil },
	}
	provided := map[string]string{GitLabUsername: "employee", GitLabToken: "token-canary"}
	values, err := resolver.ResolveProvidedForPlan(context.Background(), desired, []reconcile.Action{{Kind: reconcile.ActionSyncContent}}, provided)
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(store.saved, provided) {
		t.Fatalf("saved = %#v; want %#v", store.saved, provided)
	}
	if values[GitLabUsername] != "employee" || values[GitLabToken] != "token-canary" || values[GitCAFile] != "C:/private/ca.pem" {
		t.Fatalf("resolved values = %#v", values)
	}
}

func TestResolverResolveProvidedForPlan_RejectsUnexpectedSecret(t *testing.T) {
	resolver := Resolver{
		Home:  func(domain.DesiredState) (string, error) { return testutil.TempDir(t), nil },
		Store: func(string) (SecretStore, error) { return &fakeStore{}, nil },
	}
	_, err := resolver.ResolveProvidedForPlan(context.Background(), nonHermesDesired(t), []reconcile.Action{{Kind: reconcile.ActionSyncContent}}, map[string]string{PublicProviderAPIKey: "wrong-branch"})
	if err == nil || !strings.Contains(err.Error(), "CREDENTIAL_VALUE_INVALID") {
		t.Fatalf("error = %v; want CREDENTIAL_VALUE_INVALID", err)
	}
}

func TestRequiredNamesForPlan_ReturnsSortedPublicNames(t *testing.T) {
	for _, test := range []struct {
		name    string
		...
		{
			name:    "non-Hermes Git content",
			desired: nonHermesDesired(t),
			actions: []reconcile.Action{{Kind: reconcile.ActionSyncContent}},
			want:    []string{GitLabUsername, GitLabToken},
		},
		{
			name:    "Hermes application configuration",
			desired: hermesDesired(t),
			actions: []reconcile.Action{{Kind: reconcile.ActionConfigureApplication}},
			want:    []string{JiraToken, PublicProviderAPIKey, ConfluenceToken},
		},
		{
			name:    "Hermes Git and application configuration",
			desired: hermesDesired(t),
			actions: []reconcile.Action{{Kind: reconcile.ActionSyncDatabase}, {Kind: reconcile.ActionConfigureApplication}},
			want:    []string{GitLabUsername, JiraToken, PublicProviderAPIKey, ConfluenceToken, GitLabToken},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			if got := RequiredNamesForPlan(test.desired, test.actions); !reflect.DeepEqual(got, test.want) {
				t.Fatalf("RequiredNamesForPlan()=%v want=%v", got, test.want)
			}
		})
	}
}
...
func nonHermesDesired(t *testing.T) domain.DesiredState {
	t.Helper()
	root := testutil.TempDir(t)
	desired, err := domain.NewDesiredState(domain.DesiredStateInput{
		OS: domain.OSLinux, Application: domain.AppCodex, AppInstalled: true,
