...
func TestProfileFromDesired_AllCatalogSelectionsUseExactIdentityAndManagedMCPs(t *testing.T) {
	for _, project := range catalog.Projects() {
		for _, role := range catalog.Roles() {
			for _, toolchain := range catalog.Toolchains() {
				t.Run(string(project.ID)+"/"+string(role.ID)+"/"+string(toolchain.ID), func(t *testing.T) {
					desired, err := domain.NewDesiredState(domain.DesiredStateInput{
						OS:           domain.OSLinux,
						Application:  domain.AppHermes,
						AppInstalled: true,
						KitHome:      filepath.Join(t.TempDir(), "kit"),
					...
					if err != nil {
						t.Fatalf("new desired state: %v", err)
					}
					profile, err := ProfileFromDesired(desired)
					if err != nil {
						t.Fatalf("profile from desired: %v", err)
					}
					wantIdentity := "1c-" + string(project.ID) + "-" + string(role.ID) + "-" + string(toolchain.ID)
					if profile.Identity() != wantIdentity {
						t.Fatalf("identity = %q, want %q", profile.Identity(), wantIdentity)
					}
					if profile.Toolchain != (Toolchain{Name: string(toolchain.ID), Origin: toolchain.Origin, Version: toolchain.Commit}) {
						t.Fatalf("toolchain = %#v", profile.Toolchain)
					}
					rendered, err := profile.Render(PublicProviderProvider())
					if err != nil {
						t.Fatalf("render profile: %v", err)
					}
					for _, want := range []string{"public-development", "v8std:", "public-provider-issues:", "public-provider-wiki:"} {
						if !strings.Contains(string(rendered), want) {
							t.Fatalf("rendered profile missing %q", want)
						}
					}
				})
			}
