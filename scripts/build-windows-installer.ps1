...
function Invoke-IsccAndGetVersion {
    param(
    ...
    if ($exitCode -ne 0) { throw "INNO_BUILD_FAILED" }

    $version = $null
    foreach ($line in $output) {
        if (([string]$line) -match '^Compiler engine version: Inno Setup (6\.[0-9]+(?:\.[0-9]+){0,2})\s*$') {
            if ($version) { throw "INNO_VERSION_OUTPUT_INVALID" }
            $version = $Matches[1]
        }
    }
    if ([string]::IsNullOrWhiteSpace($version)) { throw "INNO_VERSION_OUTPUT_INVALID" }
    return $version
}
...
try {
    ...
    $InnoVersion = Invoke-IsccAndGetVersion -Iscc $Iscc -Arguments @(
        "/DAppVersion=$VersionNumber",
        "/DTeamKitExe=$Binary",
        "/DTeamKitGui=$TeamKitGui",
        "/DTeamKitBundle=$TeamKitBundle",
        "/DOutputDir=$Destination",
        $InnoScript
    )
