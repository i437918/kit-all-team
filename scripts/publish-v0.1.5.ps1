...
param(
    ...
    [string]$GitLabPipelineId,
    [string]$GitLabHandoffJobId,
    [string]$GitLabVerifyJobId,
...
function ConvertTo-EntryId([string]$Value, [string]$ReasonCode) {
    [long]$parsed = 0
    if (-not [long]::TryParse($Value, [ref]$parsed) -or $parsed -le 0) { throw [System.InvalidOperationException]::new($ReasonCode) }
    return $parsed
}
...
try {
    ...
    if ($Version -cne 'v0.1.5') { throw [System.InvalidOperationException]::new('ENTRY_VERSION_INVALID') }
    $parsedGitHubRunId = ConvertTo-EntryId $GitHubRunId 'ENTRY_GITHUB_RUN_ID_INVALID'
    $parsedGitHubArtifactId = ConvertTo-EntryId $GitHubArtifactId 'ENTRY_GITHUB_ARTIFACT_ID_INVALID'
    $parsedGitLabPipelineId = ConvertTo-EntryId $GitLabPipelineId 'ENTRY_GITLAB_PIPELINE_ID_INVALID'
    $parsedGitLabHandoffJobId = ConvertTo-EntryId $GitLabHandoffJobId 'ENTRY_GITLAB_HANDOFF_JOB_ID_INVALID'
    $parsedGitLabVerifyJobId = ConvertTo-EntryId $GitLabVerifyJobId 'ENTRY_GITLAB_VERIFY_JOB_ID_INVALID'
    if ($parsedGitLabHandoffJobId -eq $parsedGitLabVerifyJobId) { throw [System.InvalidOperationException]::new('ENTRY_GITLAB_HANDOFF_JOB_ID_CONFLICT') }
    ...
    $context = New-ReleaseContext -CandidateSha $CandidateSha -Version $Version -MaxMinutes $parsedMaxMinutes -Stopwatch $entryStopwatch -GitHubRunId $parsedGitHubRunId -GitHubArtifactId $parsedGitHubArtifactId -GitLabPipelineId $parsedGitLabPipelineId -GitLabHandoffJobId $parsedGitLabHandoffJobId -GitLabVerifyJobId $parsedGitLabVerifyJobId
