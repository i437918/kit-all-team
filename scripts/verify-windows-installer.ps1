...
if ($RequireUnsigned -and ($RequireSignature -or $ExpectedSignerThumbprint)) {
    throw "SIGNATURE_POLICY_CONFLICT"
}
...
if ($RequireUnsigned) {
    if ($Signature.Status -ne "NotSigned") {
        throw "SIGNATURE_UNEXPECTED: status=$($Signature.Status)"
    }
    if ($Manifest.signer.status -ne "NotSigned" -or $Manifest.signer.policy -ne "internal-unsigned") {
        throw "MANIFEST_UNSIGNED_POLICY_MISMATCH"
    }
}
